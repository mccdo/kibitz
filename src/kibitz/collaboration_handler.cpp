...
void collaboration_handler::operator()()
{
    ...
    if( cb )
    {
        KIBITZ_LOG_DEBUG( "Invoking collaboration callback" );
        try
        {
            ...
            {
                KIBITZ_LOG_DEBUG( "GOT COLLAB MESSAGES" );
           ...
           KIBITZ_LOG_INFO( "Invoking message handler for worker type ["
        ...
        {
            KIBITZ_LOG_ERROR( "Collaboration callback threw an exception" );
        }
    }
    else
    {
        KIBITZ_LOG_ERROR( "Recieved a message from another worker that "
